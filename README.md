...
1. основной файл `.grb`;
2. при необходимости ZIP с фрагментами, изображениями и материалами.

Windows Worker открывает GRB через T-FLEX CAD 17 Open API, перестраивает
документ и извлекает внешние и вычисляемые переменные, списки значений,
уровни видимости, формулы, ошибки перестроения и ссылки на компоненты. Поля
обогащаются проверенными паттернами LEHY/SMEC. Администратор проверяет и при
необходимости исправляет черновик, после чего публикует шаблон. Предупреждения не
блокируют публикацию. ZIP проверяется на zip-slip, символические ссылки,
исполняемые типы, лимиты файлов и распаковки. Существующие шаблоны не мигрируются.
