...
public sealed class WindowsDeploymentScriptContractTests
{
    ...
    private static readonly string HybridDeploymentPath = Path.Combine(
        RepositoryRoot,
        "scripts",
    ...
    [Fact]
    public void HybridDeploymentUsesAValidatedContainerHandoffWithNativeFallback()
    {
        var hybrid = File.ReadAllText(HybridDeploymentPath);
        ...
        var serviceUpdate = hybrid.IndexOf(
            ...
            StringComparison.Ordinal);
        var candidateCheck = hybrid.IndexOf(
            "Smoke-testing the candidate API container",
            StringComparison.Ordinal);
        var nativeDisable = hybrid.IndexOf(
            "Disable-NativeApiService",
            candidateCheck,
            StringComparison.Ordinal);
        var finalHealth = hybrid.IndexOf(
            "Assert-AuthenticationBoundary $ApiHostPort",
            nativeDisable,
            StringComparison.Ordinal);

        Assert.True(serviceUpdate >= 0);
        Assert.True(candidateCheck > serviceUpdate);
        Assert.True(nativeDisable > candidateCheck);
        Assert.True(finalHealth > nativeDisable);
    }
    ...
    public void ApiContainerUsesWindowsServerCoreNonAdminAndExternalConfiguration()
    {
        ...
        Assert.Contains(
            "mcr.microsoft.com/dotnet/sdk:10.0-windowsservercore-ltsc2022",
            dockerfile,
            StringComparison.Ordinal);
        Assert.Contains(
        ...
        Assert.Contains(
            "Environment.GetEnvironmentVariable(\"TFLEX_CONFIGURATION_FILE\")",
            program,
            StringComparison.Ordinal);
        Assert.Contains("builder.Configuration.AddJsonFile(", program, StringComparison.Ordinal);
        Assert.Contains("builder.Configuration.AddEnvironmentVariables();", program, StringComparison.Ordinal);
